...
const config: ExtendedBuidlerConfig = {
  ...
  networks: {
    ...
    mainnet: {
      ...
      accounts: [MAINNET_PRIVATE_KEY, MAINNET_PRIVATE_KEY_SECONDARY].filter((item) => item !== ""),
    },
    kovan: {
      url: `https://kovan.infura.io/v3/${INFURA_API_KEY}`,
      accounts: [KOVAN_PRIVATE_KEY, KOVAN_PRIVATE_KEY_SECONDARY].filter((item) => item !== ""),
    },
    rinkeby: {
      url: `https://rinkeby.infura.io/v3/${INFURA_API_KEY}`,
      blockGasLimit: 8000000,
      gas: 8000000,
      accounts: [RINKEBY_PRIVATE_KEY, RINKEBY_PRIVATE_KEY_SECONDARY].filter((item) => item !== ""),
    },
    coverage: {
  ...
  .setAction(async (taskArgs, { ethers }) => {
    ...
    for (const token of tokens) {
      ...
      const amount = new BigNumber(
        Math.floor(
          (((allocation.initialValue / token.value) * token.weight) / 100) *
...
task("deploy-pie-smart-pool", "deploys a pie smart pool").setAction(async (taskArgs, { ethers, run }) => {
  const signers = await ethers.getSigners();
...
task("init-smart-pool", "initialises a smart pool")
  ...
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const tx = await smartpool.init(
      taskArgs.pool,
      taskArgs.name,
      taskArgs.symbol,
      utils.parseEther(taskArgs.initialSupply)
    );
...
task("deploy-smart-pool-complete")
  .addParam(
    "balancerFactory",
    "Address of the balancer factory. defaults to mainnet balancer factory",
    "0x9424B1412450D0f8Fc2255FAf6046b98213B76Bd"
  )
  .addParam("allocation", "path to allocation")
  .setAction(async (taskArgs, { ethers, run }) => {
    // run deploy factory task
    const smartPoolFactoryAddress = await run("deploy-pie-smart-pool-factory", {
      balancerFactory: taskArgs.balancerFactory,
    });
...
task("deploy-balancer-factory", "deploys a balancer factory").setAction(async (taskArgs, { ethers }) => {
  const signers = await ethers.getSigners();
...
task("deploy-libraries", "deploys all external libraries").setAction(async (taskArgs, { ethers, deployments }) => {
  const signers = await ethers.getSigners();
...
task("deploy-libraries-and-get-object").setAction(async (taskArgs, { ethers, run }) => {
  const libraries = await run("deploy-libraries");
...
// Use only in testing!
internalTask("deploy-libraries-and-smartpool").setAction(async (taskArgs, { ethers, run, deployments }) => {
  const { deploy } = deployments;
  const signers = await ethers.getSigners();
  const libraries = await run("deploy-libraries-and-get-object");

  const contract = await deploy("PV2SmartPool", {
